...
# path setup
typeset -U path
path=("$HOME/.local/bin" "$HOME/.plenv/bin" "$PYENV_ROOT/bin" "$PNPM_HOME" "$GOPATH/bin" "$ANDROID_HOME/tools" "$ANDROID_HOME/tools/bin" "$ANDROID_HOME/platform-tools" "$XDG_DATA_HOME/JetBrains/Toolbox/scripts" "$DENO_INSTALL/bin" $path)
path+="/opt/cuda/bin"
export PATH
