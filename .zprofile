...
typeset -U path
path=("$HOME/.local/bin" "$EMSDK" "$EMSDK/upstream/emscripten" "$PNPM_HOME" "$GOPATH/bin" "$ANDROID_HOME/tools" "$ANDROID_HOME/tools/bin" "$ANDROID_HOME/platform-tools" "$XDG_DATA_HOME/JetBrains/Toolbox/scripts" $path)
export PATH
